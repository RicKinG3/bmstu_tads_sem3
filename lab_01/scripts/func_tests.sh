...
while [[ -f "func_tests/data/pos_${num_file}_in.txt" && -f "func_tests/data/pos_${num_file}_out.txt" ]]
do
    ...
    if [[ -n "$ARGS" ]]; then # если есть глобатьная переменая ARGS то в позитивные идет третий позиционный
        ARGS=$(cat "func_tests/data/pos_${num_file}_args.txt")
    fi
...
while [[ -f "func_tests/data/neg_${num_file}_in.txt" ]]
do

   
    
    if [[ -n "$ARGS" ]]; then # если есть глобатьная переменая ARGS то в позитивные идет третий позиционный
        ARGS=$(cat "func_tests/data/neg_${num_file}_args.txt")
    fi

  	./func_tests/scripts/neg_case.sh "func_tests/data/neg_${num_file}_in.txt" "$ARGS"
	value_return=$?
