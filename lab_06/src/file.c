...
int find_word_file_ex(FILE *f, tree_node_t **root, char *input_word)
{
    ...
    find_node = node_create(input_word);
    if (find_node)
    {
        int count = find_word_file(f, input_word);
        find_node->count_repeat = count;
        if (count != ERR_GETLINE)
            add_word_ex(f, root, input_word, count, find_node);
    }
    else
        return ERR_ALLOC;
    return PASS;
